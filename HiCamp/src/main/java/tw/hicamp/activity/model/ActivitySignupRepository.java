...
//	日期查詢訂單
	@Query("SELECT singupOrder FROM ActivitySignup singupOrder WHERE singupOrder.memberNo = :memberNo AND singupOrder.signupDate BETWEEN :startDate AND :endDate")
	List<ActivitySignup> findBySignupDateBetween(@Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("memberNo") Integer memberNo);
